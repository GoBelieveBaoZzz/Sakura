...
function poi_time_since( $older_date, $comment_date = false, $text = false ) {
  $chunks = array(
    array( 24 * 60 * 60, __( ' days ago', 'akina' ) ),/*天前*/
    array( 60 * 60 , __( ' hours ago', 'akina' ) ),/*小时前*/
    array( 60 , __( ' minutes ago', 'akina' ) ),/*分钟前*/
    array( 1, __( ' seconds ago', 'akina' ) )/*秒前*/
  );
 ...
 * Ajax评论
 */
if ( version_compare( $GLOBALS['wp_version'], '4.4-alpha', '<' ) ) { wp_die(__('Please upgrade wordpress to version 4.4+','akina')); }/*请升级到4.4以上版本*/
// 提示
...
function Exuser_center(){ ?>
  <script language='javascript' type='text/javascript'> 
    ...
    function gopage(url,type){ 
        URL = url; 
        if(type == 1){
          TYPE = <?php _e('dashboard','akina')/*管理后台*/?>;
        }else{
          TYPE = <?php _e('home','akina')/*主页*/?>;
        }
    ...
    function doUpdate(num){ 
        document.getElementById('login-showtime').innerHTML = '<?php _e('Login successful, ','akina')/*空降成功*/?>'+num+'<?php _e('seconds later automatically transfer to','akina')/*秒后自动转到*/?>'+TYPE;
...
function login_ok(){ 
  ...
  wp_get_current_user();
?>
  <p class="ex-login-avatar"><a href="http://cn.gravatar.com/" title="<?php _e('Change avatar','akina')/*更换头像*/?>" target="_blank" rel="nofollow"><?php echo get_avatar( $current_user->user_email, '110' ); ?></a></p>
  <p class="ex-login-username"><?php _e('Hello, ','akina')/*你好，*/?><strong><?php echo $current_user->display_name; ?></strong></p>
  <?php if($current_user->user_email){echo '<p>'.$current_user->user_email.'</p>';} ?>
  <p id="login-showtime"></p>
  <p class="ex-logout">
    <a href="<?php bloginfo('url'); ?>" title="<?php _e('Home','akina')/*首页*/?>"><?php _e('Home','akina')/*首页*/?></a>
    <?php if(current_user_can('level_10')){  ?>
    <a href="<?php bloginfo('url'); ?>/wp-admin/" title="<?php _e('Manage','akina')/*后台*/?>" target="_top"><?php _e('Manage','akina')/*后台*/?></a>
    <?php } ?>
    <a href="<?php echo wp_logout_url(get_bloginfo('url')); ?>" title="<?php _e('Logout','akina')/*登出*/?>" target="_top"><?php _e('Sign out? ','akina')/*登出？*/?></a>
  </p>
<?php 
}
...
function the_headPattern(){
  ...
  if(is_single()){
    ...
    $t .= '<h1 class="entry-title search-title"> '.sprintf( __( "Search results for \" %s \"",'akina' ), get_search_query()) ./*关于“ '.get_search_query().' ”的搜索结果*/'</h1>';
  }
  ...
  <div class="pattern-center <?php if(is_single()){echo $center;} ?>">
    <div class="pattern-attachment-img lazyload" style="background-image: url(https://upyun.rm-rf.fun/sakura/icon/orange.progress-bar-stripe-loader.svg)" data-src="<?php echo $full_image_url; ?>"> </div>
...
function the_video_headPattern_hls(){
  ...
  if(is_single()){
    ...
    $full_image_url = z_taxonomy_image_url();
    $thubm_image_url = 'https://upyun.rm-rf.fun/sakura/icon/orange.progress-bar-stripe-loader.svg';
    $des = category_description() ? category_description() : ''; // 描述
    $t .= '<h1 class="cat-title">'.single_cat_title('', false).'</h1>';
    $t .= ' <span class="cat-des">'.$des.'</span>';
  }elseif(is_search()){
...
function the_video_headPattern_normal(){
  ...
  if(is_single()){
    ...
    global $user_ID; 
    if($user_ID && current_user_can('level_10')) {
        $edit_this_post_link = '<span class="bull">·</span><a href="'.get_edit_post_link().'">'._e('EDIT','akina').'</a>';
    } else {
        $edit_this_post_link = '';
    }
    $t .= the_title( '<h1 class="entry-title">', '<button id="coverVideo-btn" class=".constant-width-to-height-ratio" onclick="coverVideo()"><i class="fa fa-pause" aria-hidden="true"></i></button></h1>', false);
    ...
    <div class="header-user-avatar">
      ...
      <div class="header-user-menu">
        ...
        <div class="user-menu-option">
          <?php if (current_user_can('level_10')) { ?>
            <a href="<?php bloginfo('url'); ?>/wp-admin/" target="_blank"><?php _e('Dashboard','akina')/*管理中心*/?></a>
            <a href="<?php bloginfo('url'); ?>/wp-admin/post-new.php" target="_blank"><?php _e('New post','akina')/*撰写文章*/?></a>
          <?php } ?>
          <a href="<?php bloginfo('url'); ?>/wp-admin/profile.php" target="_blank"><?php _e('Profile','akina')/*个人资料*/?></a>
          <a href="<?php echo wp_logout_url(get_bloginfo('url')); ?>" target="_top"><?php _e('Sign out','akina')/*退出登录*/?></a>
        </div>
      </div>
    </div>
  <?php
  }else{ 
    $ava = 'https://cdn.jsdelivr.net/gh/qq1041663097/cdn@1.0.0/img/Sakura/images/none.png';
...
function siren_private_message_hook($comment_content , $comment){
    ...
    if ( $is_private ) return '<i class="fa fa-lock" aria-hidden="true"></i> '.__('The comment is private','akina')/*该评论为私密评论*/;
...
function siren_get_useragent($ua){
  if(akina_option('open_useragent')){
    ...
    $imgurl = 'https://cdn.jsdelivr.net/gh/qq1041663097/cdn@1.0.0/img/Sakura/images/ua/svg/';
