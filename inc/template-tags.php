...
function akina_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'akina' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'akina' ),
...
function akina_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( __( ', ', 'akina' ) );
		if ( $categories_list && akina_categorized_blog() ) {
			printf( '<span class="cat-links">' . __( 'Posted in %1$s', 'akina' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'akina' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . __( 'Tagged %1$s', 'akina' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
