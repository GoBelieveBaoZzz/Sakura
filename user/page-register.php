...
if( !empty($_POST['register_reg']) ) {
	...
	if ( $sanitized_user_login == '' ) {
	  $error .= '<strong>'.__('Error','akina')./*错误*/'</strong>：'.__('Please enter username.','akina')./*请输入用户名。*/'<br />';
	} elseif ( ! validate_username( $sanitized_user_login ) ) {
	  $error .= '<strong>'.__('Error','akina')./*错误*/'</strong>：'.__('Invalid characters, please enter a valid username.','akina')./*此用户名包含无效字符，请输入有效的用户名。*/'<br />';
	  $sanitized_user_login = '';
	} elseif ( username_exists( $sanitized_user_login ) ) {
	  $error .= '<strong>'.__('Error','akina')./*错误*/'</strong>：'.__('This username has been registered.','akina')./*该用户名已被注册。*/'<br />';
	}

	// Check the e-mail address
	if ( $user_email == '' ) {
	  $error .= '<strong>'.__('Error','akina')./*错误*/'</strong>：'.__('Please enter email address.','akina')./*请填写电子邮件地址。*/'<br />';
	} elseif ( ! is_email( $user_email ) ) {
	  $error .= '<strong>'.__('Error','akina')./*错误*/'</strong>：'.__('Invalid email address.','akina')./*电子邮件地址不正确。*/'<br />';
	  $user_email = '';
	} elseif ( email_exists( $user_email ) ) {
	  $error .= '<strong>'.__('Error','akina')./*错误*/'</strong>：'.__('This email address has been registered.','akina')./*该电子邮件地址已经被注册。*/'<br />';
	}

	// Check the password
	if(strlen($_POST['user_pass']) < 6){
	  $error .= '<strong>'.__('Error','akina')./*错误*/'</strong>：'.__('Password length is at least 6 digits.','akina')./*密码长度至少6位。*/'<br />';
	}elseif($_POST['user_pass'] != $_POST['user_pass2']){
		$error .= '<strong>'.__('Error','akina')./*错误*/'</strong>：'.__('Inconsistent password entered twice.','akina')./*两次输入的密码不一致。*/'<br />';
	}

	// verification
	if(akina_option('login_validate') && strlen($_POST['verification']) > 0 ){
		$error .= '<strong>'.__('Error','akina')./*错误*/'</strong>：'.__('Please drag the slider to verify identity','akina')./*请拖动滑块验证身份*/'<br />';
	}

	if($error == '') {
		$user_id = wp_create_user( $sanitized_user_login, $_POST['user_pass'], $user_email );
		if ( !$user_id ) {
			$error .= '<strong>'.__('Error','akina')./*错误*/'</strong>：'.__('Unable to complete registration request...Please contact','akina')./*无法完成注册请求... 请联系*/'<a href=\"mailto:'. get_option( 'admin_email' ) .'\">'.__('administrator','akina')./*管理员*/'</a>！<br />';
...
?>
	<div id="primary" class="content-area">
				...
				<form action="<?php echo $_SERVER["REQUEST_URI"]; ?>" method="post">  
					...
					<div id="verification-slider">
						<div id="slider">
							...
							<span id="label">»</span><span id="labelTip"><?php _e('Drag the slider to verify','akina')/*拖动滑块验证*/?></span>
						</div>
					...
					<input class="button register-button" name="submit" type="submit" value="<?php _e('Sign up','akina')/*注 册*/?>">
				</form>
			</div>
		<?php }else{ 
		...
		?>
			<div class="ex-register-title">
				<h3><?php _e('Success! Redirecting......','akina')/*注册成功！正在跳转...*/?></h3>
			</div>
			<script>window.location.href='<?php echo $loadurl; ?>';</script>
		<?php } ?>
		<?php else : ?>
			<div class="register-close"><p><?php _e('Registration is not open yet.','akina')/*暂未开放注册。*/?></p></div>
		<?php endif; ?>
		</main><!-- #main -->
	</div><!-- #primary -->
<style type="text/css">
